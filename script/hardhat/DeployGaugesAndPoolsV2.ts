...
async function main() {
  ...
  for (var i = 0; i < jsonConstants.poolsV2.length; i++) {
    ...
    await factory.createPair(tokenA, tokenB, stable, { gasLimit: 5000000 });
    let pool = await factory.getPair(tokenA, tokenB, stable, {
      gasLimit: 5000000,
    });
  ...
  for (var i = 0; i < jsonConstants.poolsVeloV2.length; i++) {
    const { stable, token } = jsonConstants.poolsVeloV2[i];
    await factory.createPair(deployedContracts.VELO, token, stable, {
      gasLimit: 5000000,
    });
    let pool = await factory.getPair(deployedContracts.VELO, token, stable, {
      gasLimit: 5000000,
    });
    await voter.createGauge(
      deployedContracts.poolFactory, // PoolFactory (v2)
